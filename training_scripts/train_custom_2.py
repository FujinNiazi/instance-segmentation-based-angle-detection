...
# ---- Step A: Custom Output Layer with Weighted Classification Loss ----
class WeightedFastRCNNOutputLayers(FastRCNNOutputLayers):
    def __init__(self, cfg, input_shape):
        super().__init__(cfg, input_shape)
        # Class weights (camera, line, robot)
        self.class_weights = torch.tensor([1.0, 1.5, 10], device=self.cls_score.weight.device)
    ...
    def losses(self, predictions, proposals):
        ...
        loss_cls = F.cross_entropy(scores, gt_classes, weight=self.class_weights)
...
class CustomROIHeads(StandardROIHeads):
    ...
    def mask_rcnn_loss(self, pred_mask_logits, instances):
        """
        Override mask loss to add per-class weighting.
        """
        if len(instances) == 0:
        ...
        loss = F.binary_cross_entropy_with_logits(pred_masks, gt_masks, weight=weights, reduction="mean")
...
class MyCustomTrainer(MyTrainer):
    ...
    def build_model(cls, cfg):
        ...
        print("[INFO] Using custom ROI heads with class-weighted loss")
        return model
...
# # Improve fine detail preservation (especially for lines)
# cfg.INPUT.MIN_SIZE_TRAIN = (1080,)     # Avoid downscaling
# cfg.INPUT.MAX_SIZE_TRAIN = 1920
# cfg.INPUT.MIN_SIZE_TEST = 1080

# cfg.INPUT.FORMAT = "BGR"  # Input format

# ---- Augmentations for Training ----
# augmentation = [
#     # T.RandomRotation(angle=[-10, 10]),      # Rotate by -10 to 10 degrees
#     # T.RandomFlip(prob=0.5), # Random horizontal flip
#     # T.RandomFlip(prob=0.4, vertical=True, horizontal=False),   # Random vertical flip
#     # T.Resize((800, 800)),   # Ensure consistent image size


#     # Less aggressive crop to preserve thin lines
#     # T.RandomCrop("relative", (0.9, 0.9)),

#     # Mild brightness/contrast/saturation variation
#     # T.RandomBrightness(0.95, 1.05)
#     # T.RandomContrast(0.95, 1.05),
#     # T.RandomSaturation(0.95, 1.05),
#     # T.RandomLighting(0.7),

#     # Optional: Mild blur to generalize fine textures
#     #T.RandomApply(T.GaussianBlur((0.1, 1.0)), prob=0.3),
# ]
